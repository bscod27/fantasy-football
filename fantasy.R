...
for (i in files) {
  ...
  # assign(x = i, value = df, envir = .GlobalEnv) # optional step as I want to see dfs in environment
  main <- rbind(main, df)
}

main <- 
  
dfff <- read.csv("~/05. Scripts/06. Fantasy Football/data/2020_special.csv")


file_1970_2019 <- dir_info() %>%
  filter(endsWith(path, "[0-9].csv")) %>%
  select(path) %>% 
    mutate(file_1970_2019 = map(path, read_csv)) %>%
    unnest() %>% 
      mutate(Year = gsub(".csv","",path)) %>% 
      mutate(Year = as.double(Year)) %>% 
      mutate(PPG = FantasyPoints/G) %>% 
      select(Player, Tm, Pos, Year, PPG)

colnames(file_1970_2019) <- tolower(colnames(file_1970_2019))

file_2020 <- read_csv("C:/Users/bscod/Documents/05. Scripts/06. Fantasy Football/2020_special.csv") %>% 
  rename(ppg = `FPTS/G`, player = NAME, tm = TEAM, pos = POS) %>% 
  select(player, tm, pos, ppg) 

file_2020$year <-  2020

master <- bind_rows(file_1970_2019, file_2020) ###### may want to get rid of all old data
...
write_csv(output, 'C:/Users/bscod/Documents/05. Scripts/06. Fantasy Football/output.csv')
#####